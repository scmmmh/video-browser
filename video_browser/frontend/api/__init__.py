...
from video_browser.models import db_session, Playlist, Video
...
class PlaylistModel(BaseModel):
    ...
    def convert_videos_to_ids(cls, value: list[Video]) -> list[str]:
        ...
        return [v.public_id for v in value]
...
async def get_playlist(pid: str, dbsession: Annotated[AsyncSession, Depends(db_session)]) -> Playlist:
    ...
    query = select(Playlist).filter(Playlist.public_id == pid).options(selectinload(Playlist.videos))
...
async def get_videos(
    ...
    query = select(Video).join(Video.playlists).filter(Playlist.public_id == pid)
